...
// 폼 제출 처리 (정적 환경에서는 비활성화)
async function handleFormSubmit(event) {
    event.preventDefault();
    
    alert('이 기능은 정적 GitHub Pages 버전에서는 지원되지 않습니다.\n\n개발 환경에서만 앱 등록이 가능합니다.');
    return;
...
async function loadApps() {
    try {
        // 정적 JSON 파일에서 데이터 가져오기 (GitHub Pages 대응)
        let response;
        try {
            response = await fetch('./data/apps.json');
        } catch (e1) {
            try {
                response = await fetch('data/apps.json');
            } catch (e2) {
        ...
        console.log(`총 ${allApps.length}개 앱 로드됨 (정적 데이터)`);
        ...
        showErrorMessage(`앱 목록을 불러오는데 실패했습니다: ${error.message}`);
...
function sortAppsBySubscription(apps) {
    // 로컬스토리지에서 즐겨찾기 상태 가져오기
    const localFavorites = JSON.parse(localStorage.getItem('app_favorites') || '{}');
    ...
    return apps.sort((a, b) => {
        ...
        // 2순위: 즐겨찾기 (로컬스토리지 우선, 같은 구독 상태 내에서 즐겨찾기 우선)
        const aFavorite = localFavorites[a.id] !== undefined ? localFavorites[a.id] : a.is_favorite;
        const bFavorite = localFavorites[b.id] !== undefined ? localFavorites[b.id] : b.is_favorite;
...
function createAppCard(app) {
    // 로컬스토리지에서 즐겨찾기 상태 확인
    const localFavorites = JSON.parse(localStorage.getItem('app_favorites') || '{}');
    const isFavorite = localFavorites[app.id] !== undefined ? localFavorites[app.id] : app.is_favorite;
    
    // 로컬스토리지에서 사용 횟수 확인
    const localUsage = JSON.parse(localStorage.getItem('app_usage') || '{}');
    const usageCount = localUsage[app.id] || app.usage_count || 0;
    ...
    return `
        <div class="app-card p-6">
            <div class="flex items-start justify-between mb-4">
                ...
                <button onclick="toggleFavorite('${app.id}', ${isFavorite})" 
                        class="favorite-btn ${isFavorite ? 'active' : ''} text-gray-400 hover:text-red-500">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs text-gray-500">사용 ${usageCount}회</span>
            </div>
            ...
            <div class="flex space-x-2">
                ...
                <button onclick="alert('편집 기능은 정적 버전에서 지원되지 않습니다.')" 
                        class="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm opacity-50" 
                        title="정적 버전에서는 편집 불가">
                    <i class="fas fa-edit"></i>
                </button>
                <button onclick="alert('삭제 기능은 정적 버전에서 지원되지 않습니다.')" 
                        class="px-3 py-2 border border-gray-300 rounded-lg hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition-colors text-sm opacity-50"
                        title="정적 버전에서는 삭제 불가">
...
function openApp(appId, url) {
    try {
        console.log('앱 실행:', url);
        
        // 새 창에서 앱 열기
        window.open(url, '_blank');
        
        // 로컬스토리지에 사용 기록 저장 (선택사항)
        try {
            const usageData = JSON.parse(localStorage.getItem('app_usage') || '{}');
            usageData[appId] = (usageData[appId] || 0) + 1;
            localStorage.setItem('app_usage', JSON.stringify(usageData));
        } catch (storageError) {
            console.log('사용 기록 저장 실패:', storageError);
        }
...
// 즐겨찾기 토글 (정적 환경용 - 로컬스토리지 사용)
function toggleFavorite(appId, currentStatus) {
    try {
        // 로컬스토리지에 즐겨찾기 상태 저장
        const favorites = JSON.parse(localStorage.getItem('app_favorites') || '{}');
        favorites[appId] = !currentStatus;
        localStorage.setItem('app_favorites', JSON.stringify(favorites));
        
        // 화면 즉시 업데이트
        loadApps();
        
        console.log(`즐겨찾기 ${!currentStatus ? '추가' : '제거'}: ${appId}`);
...
async function filterApps() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('앱 데이터를 불러오는데 실패했습니다.');
        }
